  ...
  return (
    <section className="relative z-10 px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
      <div className="mx-auto max-w-7xl sm:px-6 sm:py-4 lg:px-8">
        ...
        >
          <Sparkles className="hidden h-6 w-6 shrink-0 sm:block sm:h-7 sm:w-7" />
          <h2>One FaceDeliver - Many Solutions</h2>
        </motion.div>

        {/* 2-col grid on mobile/tablet → 4-col on lg */}
        <div className="grid grid-cols-2 gap-x-4 gap-y-8 sm:gap-10 lg:grid-cols-4 lg:gap-8">
          {solutions.map((solution, index) => (
            ...
            >
              ...
              {/* Portrait image */}
              <div
                className={`relative aspect-[5/7] w-full max-w-[10rem] overflow-hidden rounded-t-[5.5rem] bg-gradient-to-br ${solution.bg} sm:aspect-auto sm:h-64 sm:w-48 sm:max-w-none md:h-72 md:w-52 lg:w-56`}
              >
                <Image
                  src={solution.image}
                  alt={solution.title}
              ...
              {/* Label pill */}
              <div className="mt-4 w-full max-w-[10rem] rounded-xl bg-zinc-900 px-3 py-2.5 text-center text-xs font-medium text-white shadow-lg sm:w-auto sm:max-w-none sm:px-6 sm:py-3 sm:text-base lg:rounded-xl lg:px-7">
                {solution.title}
              </div>
            </motion.article>
          ))}
