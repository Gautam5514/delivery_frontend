...
export default function Sidebar({
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-50 w-64 transform overflow-hidden border-r border-zinc-200/80 bg-white/75 shadow-[18px_0_50px_-35px_rgba(24,24,27,0.45)] backdrop-blur-2xl transition-[width,transform,box-shadow] duration-300 ease-out md:sticky md:top-0 md:h-screen md:translate-x-0 md:flex-shrink-0 ${
    ...
    >
      ...
      <div
        className={`flex h-20 items-center border-b border-zinc-200/60 px-5 transition-all duration-300 ${
          isCollapsed ? "md:justify-center md:px-3" : "justify-between"
        }`}
      >
        <Link
        ...
        {/* Mobile close */}
        <button
          className="text-zinc-400 transition-colors hover:text-zinc-600 md:hidden"
          onClick={onClose}
          type="button"
        ...
        <button
          className={`hidden h-9 w-9 items-center justify-center rounded-full bg-white text-zinc-500 cursor-pointer md:flex ${
            isCollapsed ? "md:absolute md:left-1/2 md:top-24 md:-translate-x-1/2" : ""
          }`}
      ...
      <div
        className={`flex h-[calc(100vh-5rem)] flex-col justify-between overflow-y-auto transition-all duration-300 ${
          isCollapsed ? "p-3 pt-16 md:p-3 md:pt-16" : "p-4"
        }`}
      >
